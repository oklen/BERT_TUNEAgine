...
def main():
    ...
    parser.add_argument('--generte_count', type=int, default=100)
    ...
    for input_path in glob(args.input_pattern):
        ...
        with gzip.GzipFile(fileobj=tf.gfile.GFile(input_path, "rb")) as fi:
            ...
            for line in fi:
                ...
                if is_training:
                    #for token in nlp(json_example["question_text"]):
                    #    question_tokens.append(token)
                    question_tokens = json_example["question_text"].split(" ")
                    #mnlp = nlp
                    #chtml = re.compile(r'''^<.*?>$''').match
                    #mnlp.tokenizer = Tokenizer(mnlp.vocab,token_match=chtml)
