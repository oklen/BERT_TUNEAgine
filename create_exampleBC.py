...
class InputFeatures(object):
    ...
    def show(self):
        ...
        print(self.graph)
        print("answer_type:",self.answer_type)
        print("start_positions:",self.start_positions)
        print("end_positions:",self.end_positions)
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for candidate_idx, candidate in sorted_la_candidates:
            ...
            all_doc_tokens.append(
                "[unused{}]".format(
                    get_candidate_type(example.doc_tokens[start_index]["token"], counts, 50)))
...
def main():
    ...
    prefix = "cached_{0}_{1}_{2}_{3}".format(str(args.max_seq_length), str(args.doc_stride), str(args.max_query_length)
    ,Frame_Name)
        ...
        with gzip.GzipFile(fileobj=tf.gfile.GFile(input_path, "rb")) as fi:
            ...
            for line in fi:
                ...
                if is_training:
                    ...
                    question_tokens = json_example["question_text"]

                    tmp_doc_tokens = json_example["document_text"].split(" ")
                    is_html = re.compile(r'''<.*?>''').match
                    for i in tmp_doc_tokens:
                        tmp_dict = {}
                        tmp_dict['token'] = i
                        if is_html(i):
                            tmp_dict['html_token'] = True
                        else:
                            tmp_dict['html_token'] = False
                        doc_tokens.append(tmp_dict)
                else:
                    logging.info("Process DEV dataset is not implemented!")
                    exit(0)
