...
from modules.graph_encoderLL import NodePosition, Graph, EdgeType, get_edge_position
...
def convert_examples_to_features(args, examples, tokenizer, is_training, cached_path):
    ...
    for (example_index, example) in enumerate(examples):
        ...
        r"""
        Below is seems to process all the text that content la_candidates,and cut them into
        tokens,sentences and paragraph, and build some record
        """
        
        start_index = 0
        end_index = len(example.doc_tokens)-1
        ...
        for tok_idx in range(start_index, end_index):
            ...
            context_tokens.append(token_item.strip())
            if context_tokens[-1] == "":
        ...
        r""""
        Below code find the long and short answer postion,with modify text idx
        """

        # The -3 accounts for [CLS], [SEP] and [SEP]
        
        max_tokens_for_doc = args.max_seq_length - 2
        ...
        #logging.info("doc_spans:{} len{}".format(doc_spans,len(doc_spans)))
        #exit(0)
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            #print(len(tokens),len(is_sentence_end))
            ...
            doc_tree = get_doc_tree(is_sentence_end, is_paragraph_end, orig_tok_idx)

            graph = build_graph(doc_tree, len(is_sentence_end))
...
def main():
    ...
    for input_path in glob(args.input_pattern):
        ...
        train = pd.read_csv(args.input_pattern, usecols=['Plot', 'Genre'])
        if args.start_num == -1:args.start_num=0
        if args.end_num == -1:args.end_num = 30000
        
        my_kinds = ['action','adventure','comedy','drama']
#  action 1098
#  adventure 526
#  cartoon 1
#  comedy 4376
...
#sci-fi 221
        for i in range(args.start_num,args.end_num):
            if train['Genre'][i] not in my_kinds: continue
            index = 0
            for j,name in enumerate(my_kinds):
                if train['Genre'][i]==name:
                    index=j
            examples.append(NqExample(train['Plot'][i].split(),index))
        
        logging.info("Generate Example {}".format(len(examples)))
