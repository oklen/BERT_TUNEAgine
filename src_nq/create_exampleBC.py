...
def convert_examples_to_features(args, examples, tokenizer, is_training, cached_path):
    ...
    for (example_index, example) in enumerate(examples):
        ...
        query_tokens = tokenizer(query_tokens)['input_ids']
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            tokens = tokens[tokens_length:]
            tokens = tokenizer(tokens)['input_ids']
            tokens = tokens[1:]
...
def main():
    ...
    Frame_Name = "albert-base-v2"
    tokenizer = AutoTokenizer.from_pretrained(Frame_Name)
