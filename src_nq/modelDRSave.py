...
from modules.graph_encoderDr2 import NodeType, NodePosition, EdgeType, Encoder,GraphEncoder
...
class NqModel(BertPreTrainedModel):
    def __init__(self, bert_config, my_config):
        ...
        self.my_mask = None
        self.bert =  AutoModel.from_pretrained("mfeb/albert-xxlarge-v2-squad2")
...
#        self.my_mask = 
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, st_mask, edges, label):
...
#        print("BEFORE GRAPH:",sequence_output.shape)
        ...
        graph_output = self.encoder2(graph_output,st_mask,edges, output_all_encoded_layers=False)[0]
...
#        print(graph_output.shape,self.config.hidden_size)
        tok_logits = self.tok_outputs(torch.tanh(self.tok_dense(graph_output))).squeeze(-1)
#        tok_logits = tok_logits.view(tok_logits.size(0),tok_logits.size(1))
#        print(st_mask.shape,tok_logits.shape)
#        if self.my_mask == None:
#            self.my_mask = torch.cat([torch.ByteTensor([0,0],device=st_mask.device),torch.ByteTensor((self.my_config.max_token_len-2)*[1],device=st_mask.device)])
#        tok_logits.masked_fill_(self.my_mask, -10000)
