...
from modules.graph_encoderRo import NodeType, NodePosition, EdgeType, Encoder,GraphEncoder
...
class NqModel(BertPreTrainedModel):
    def __init__(self, bert_config, my_config):
        ...
        self.tok_outputs = nn.Linear(self.config.hidden_size, 128)
...
#        para_logits.masked_fill_(para_zero_mask, -10000)
        ...
        # token
        ans = 0
        if tok_logits[0][0] < tok_logits[0][1]: ans = 1
        if ans == label[0]: self.right = self.right+1
        self.all = self.all+1
        print("ACC:{}".format(1.0*self.right/self.all))
