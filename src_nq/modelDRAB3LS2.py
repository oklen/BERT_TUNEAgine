...
from modules.graph_encoderABDUG4LS2V import NodeType, NodePosition, EdgeType, Encoder,GraphEncoder
...
class NqModel(nn.Module):
    def __init__(self, my_config,args):
        ...
        self.my_mask = None
        self.args = args
        self.bert_config = AlbertConfig.from_pretrained("albert-xlarge-v2")
...
#        self.doc_dense = nn.Linear(self.config.hidden_size, self.config.hidden_size)
        ...
        self.tok_outputs = nn.Linear(my_config.hidden_size*4, 1) # tune to avoid fell into bad places
...
#                    x = self.dropout(graph_output)
                    ...
                    output_scores = torch.bmm(Output,sequence_output[:,0].unsqueeze(1).transpose(-1,-2))
