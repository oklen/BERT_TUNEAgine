...
from modules.graph_encoderABDUG4LS import NodeType, NodePosition, EdgeType, Encoder,GraphEncoder
        ...
        for input_ids, attention_mask, token_type_ids, st_mask, label,edges_src, edges_tgt, edges_type, edges_pos,all_sen in zip(input_idss, attention_masks, token_type_idss, st_masks, labels,edges_srcs, edges_tgts, edges_types, edges_poss,all_sens):
            if self.args.run_og:
                if input_embs==None:
                    ...
                    sequence_output,_ = self.bert(input_ids,  attention_mask,token_type_ids,inputs_embeds=input_embs.long())
