...
#        self.paragraph_attention = GraphAttention(config, config.max_paragraph_len, config.max_paragraph_relative)
    ...
    def forward(self, input_tensor, st_mask, edges):
        ...
        graph_output = self.integration(input_tensor, edges)
...
#        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(config.num_hidden_layers)])
        ...
        self.conv = GraphConv(config.hidden_size,config.hidden_size)
...
#        hidden_states = self.initializer(hidden_states, st_mask, edges)
        ...
        mid_edge = edges_type.eq(EdgeType.SENTENCE_TO_QA).nonzero().view(-1).tolist() + edges_type.eq(EdgeType.QA_TO_SENTENCE).nonzero().view(-1).tolist()
        down_edge = edges_type.eq(EdgeType.SENTENCE_TO_TOKEN).nonzero().view(-1).tolist()
        
        up_edge = (edges_src[up_edge], edges_tgt[up_edge], edges_type[up_edge], edges_pos[up_edge])
        mid_edge = (edges_src[mid_edge], edges_tgt[mid_edge], edges_type[mid_edge], edges_pos[mid_edge])
        down_edge = (edges_src[down_edge], edges_tgt[down_edge], edges_type[down_edge], edges_pos[down_edge])
        
        all_encoder_layers = [self.layer[0](hidden_states, st_mask, up_edge)]
        all_encoder_layers[0] = self.layer[1](hidden_states,st_mask,mid_edge)
        all_encoder_layers[0] = self.layer[2](hidden_states,st_mask,down_edge)
