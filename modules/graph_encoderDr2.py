...
from torch_geometric.nn import GraphConv,AGNNConv,FastRGCNConv
...
#        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(config.num_hidden_layers)])
        ...
        self.conv = AGNNConv(config.hidden_size,config.hidden_size)
...
#        down_edge = edges_type.eq(EdgeType.SENTENCE_TO_TOKEN).nonzero().view(-1).tolist()
        ...
        x = self.conv2(x,torch.stack([edges_src[up_edge],edges_tgt[up_edge]]))
        x = self.conv(x,edge_indce)
