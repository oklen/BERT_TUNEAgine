...
class GraphAttentionLayer(nn.Module):
    def __init__(self, config):
        ...
        tmp_model_config = AutoModel.from_pretrained("allenai/longformer-base-4096").config
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        print("Graph LAYERS:",config.num_hidden_layers)
...
#        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(3)])
    ...
    def forward(self, hidden_states, st_mask, edges, output_all_encoded_layers=True):
        ...
        st_mask = st_mask - torch.ones_like(st_mask)
        #Modify st_mask to use LongFormer attention
